...
from diff.lab.controllers.controller_diff_cfg import LVControllerCfg
...
class ActionsCfg:
    ...
    force_torque = mdp.DiffActionCfg(asset_name="robot", command_type="LVController", controller_cfg=LVControllerCfg())
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        last_action = ObsTerm(func=mdp.last_action, params={"action_name": "force_torque"})
...
class EventCfg:
    reset_base = EventTerm(
        ...
        mode="reset",
        params={
            "pose_range": {"x": (-0.1, 0.1), "y": (-0.1, 0.1),"z":(1.0, 2.0),"roll": (-0.5, 0.5), "pitch": (-0.5, 0.5), "yaw": (-3.14, 3.14)},
            "velocity_range": {
                "x": (-0.0, 0.0),
                "y": (-0.0, 0.0),
                "z": (-0.0, 0.0),
...
@configclass
class QuadcopterReachTargetLVEnvCfg(ManagerBasedDiffRLEnvCfg):
