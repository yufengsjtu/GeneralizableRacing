...
def run(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    # controller
    controller_cfg = PSControllerCfg(
        rate_gain=[150.0, 150.0, 150.0],
        pose_gain=[20.0, 20.0, 20.0],
        speed_gain=[5.0, 5.0, 5.0],
        pos_gain=[3.0, 3.0, 3.0]
    )
    ...
    while simulation_app.is_running():
        ...
        states_list.append(states['pos'][0])
...
if __name__ == "__main__":
    ...
    plt.figure()
    plt.subplot(4, 1, 1)
    plt.plot(t_list, cmd_w[:, 1], label="cmd px")
    plt.plot(t_list, state_w[:, 0], label="Real px")
    plt.legend()
    plt.subplot(4, 1, 2)
    plt.plot(t_list, cmd_w[:, 2], label="cmd py")
    plt.plot(t_list, state_w[:, 1], label="Real py")
    plt.legend()
    plt.subplot(4, 1, 3)
    plt.plot(t_list, cmd_w[:, 3], label="cmd pz")
    plt.plot(t_list, state_w[:, 2], label="Real pz")
    plt.legend()
    plt.subplot(4, 1, 4)
