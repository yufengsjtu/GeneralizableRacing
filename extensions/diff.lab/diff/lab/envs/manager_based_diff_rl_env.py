...
from diff.lab.managers import LossManager, DiffActionManager
...
class ManagerBasedDiffRLEnv(ManagerBasedRLEnv, gym.Env):
    ...
    cfg: ManagerBasedDiffRLEnvCfg
    """Configuration for the environment."""

    def __init__(self, cfg: ManagerBasedDiffRLEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def load_managers(self):
        ...
        # -- loss manager
        self.loss_manager= LossManager(self.cfg.losses, self)
        print("[INFO] Loss Manager: ", self.loss_manager)
        # -- curriculum manager
        self.curriculum_manager = CurriculumManager(self.cfg.curriculum, self)
        print("[INFO] Curriculum Manager: ", self.curriculum_manager)

        # --reload action manager [NOTE - this is needed for differentiable physics]
        # Detach action' gradient when logging previous action
        self.action_manager = DiffActionManager(self.cfg.actions, self)
    ...
    def setup_manager_visualizers(self):
        ...
        self.manager_visualizers = {
            ...
            "loss_manager": ManagerLiveVisualizer(manager=self.loss_manager),
        }
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        # align dynamics if needed
        if self.cfg.is_differentiable_physics:
            for name, item in self.action_manager._terms.items():
                if hasattr(item, "align_dynamics"):
                    aligned_states, nominal_states, acc = item.align_dynamics()
                    self.extras["aligned_states"] = aligned_states
                    self.extras["nominal_states"] = nominal_states
                    self.extras["acc"] = acc
        # post-step:
        ...
        # # -- record accumulated gates
        # self.extras["accumulate_gates"] = self.command_manager.get_term("next_gate_pose").metrics["accumulate_gates"].clone()
        ...
        # -- loss computation
        if self.cfg.is_differentiable_physics:
            self.extras["losses"], self.extras["losses_detached"] = self.loss_manager.compute(
                dt=self.step_dt, 
                aligned_states=self.extras["aligned_states"].clone(),
                action=self.extras["acc"].clone()
                )
            self.extras["log_losses"] = self.loss_manager.log_all_active_terms()
    ...
    def _reset_idx(self, env_ids: Sequence[int]):
        ...
        self.extras["log"].update(info)
        # -- loss manager
        info = self.loss_manager.reset(env_ids)
        self.extras["log"].update(info)
        # -- curriculum manager
    ...
    def detach(self,):
        # detach all action terms
        for term in self.action_manager._terms.values():
            if hasattr(term, "detach"):
                term.detach()